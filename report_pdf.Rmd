...
We test the target-related variables q25 to q29 for potential pattern(s) in the target related missing data. This results in a p-value of 0, which indicates, that the data is not missing completely at random. Omitting all NAs can be problematic and lead to bias. But imputing the target variable related data may be quite challenging and out of the scope of this project. We therefore decide to nevertheless drop the target related NAs.
...
RISK_reduced[, c("q6orig", "q7orig")] <- NULL
...
  Information Gain can be used in the construction of decision trees but also for feature selection [[3]](#3). Therefore the informtaion gain for each variable with regard to the target variable is calculated.
...
RISK_red <- convert_columns_to_factors("RISK_red")
...
\
A linear Kernel $$ K(u,v) = \langle \mathbf u, \mathbf v \rangle = \sum_{j=1}^{p}u_{j}v_{j}$$ a polynomial Kernel $$ K(u,v) = (c + \langle \mathbf u, \mathbf v \rangle)^{d}, \ with \ d > 1$$\
...
\
Naive Bayes is quite simple and only has one hyperparameter that we need to consider: the use of Laplace smoothing. The type of Naive Bayes model is automatically estimated by our package (naivebayes).\
Laplace smoothing is used to obtain "more smooth" class distribution and avoid zero probabilities. It is a numeric parameter. We examine three common values for that parameter: 0 (leading to Max-likelihood estimation), 0.5 and 1.

```{r training naive bayes, include=FALSE, cache=TRUE}
...
train_nb <- function(dataset) {
    train(suicidal_class ~ ., data = dataset,
                   method = 'naive_bayes',
                   tuneGrid = data.frame(laplace = laplace, usekernel = FALSE, adjust = FALSE),
                   metric="Accuracy",
                   preProcess = c("center"),
...
```{r plot cross-val hpo results, echo=FALSE, cache=TRUE}
plot(nb_best_model)
#print(nb_best_model)
```
...
# save all models
saveRDS(model_linear, file = "model_linear.rda")
saveRDS(model_radial, file = "model_radial.rda")
saveRDS(model_poly, file = "model_poly.rda")
...
svm_best_model_m <- svm_models[[svm_best_model_name]]
# Extract the best hyperparameters
svm_best_hyperparameters <- svm_best_model_m$bestTune

svm_best_model <- svm_best_model_m$finalModel
...
```{r Confusion Matrix of SVM,echo=FALSE, cache=TRUE}
svm_best_model <- readRDS("svm_best_model.rda")

# rename target levels 
...
classifier_metrics <- ml_test(nb_best_model_pred, scaledRISK_test$suicidal_class, output.as.table = FALSE)
...
nb_classifier_metrics <- ml_test(nb_best_model_pred, scaledRISK_test$suicidal_class, output.as.table = TRUE)
...
```{r svm multiclass metrics, echo=FALSE, cache=TRUE}
library(mltest)

classifier_metrics <- ml_test(svm_best_model_pred, svm_actual, output.as.table = FALSE)
...
# tabular view of the metrics (except for 'accuracy' and 'error.rate')
classifier_metrics <- ml_test(svm_best_model_pred, svm_actual, output.as.table = TRUE)
...
```{r NB no scaling, echo=FALSE, cache=TRUE, eval=FALSE}
set.seed(0)  # set the seed
...
```{r plot best model performance, echo=FALSE, cache=TRUE}
# Print the name and details of the best model
# print(svm_no_sc_best_model)
plot(model_nb$finalModel)
```
...
```{r ROC curves, include=FALSE, cache=TRUE }
library(pROC)

# get predictions from best model
nb_best_model_pred <- predict(nb_best_model, scaledRISK_test_no_target, type = "prob")

# binarize output
testing_res_binary <- model.matrix(~ scaledRISK_test$suicidal_class - 1)

roc_multiclass <- multiclass.roc(response = scaledRISK_test$suicidal_class, predictor = nb_best_model_pred,
                                 levels = c("l1", "l2", "l3"))

rs <- roc_multiclass[['rocs']]

# TODO: plot roc curves
```

#### 5.4.2 ROC for best SVM model

To plot the ROC curves, we need to get the probabilities for our model predictions. Since we set a seed, we can retrain on our best model and its hyperparameters to get the probabilities. Unfortunately, this was not possible as an upstream task. todo:

```{r ROC curves, include=FALSE, cache=TRUE }
library(pROC)
library(nnet)  

train_control_probs <- trainControl(method = "cv",
                              number = 5, 
                              search = "grid",
                              classProbs=TRUE)
          
train_svm_probs <- function(dataset, kernel, grid) {
    train(suicidal_class ~ ., data = dataset,
                   method = kernel,
                   tuneGrid = grid, 
                   metric="Accuracy",
                   preProcess = c("center"),
                   trControl = train_control_probs)
}

# retrain best model with best hp config for probabilities
set.seed(0)
model_radial_probs <- train_svm_probs(scaledRISK_train, "svmRadial", grid_radial)

# get predictions from best model
model_radial_probs_pred <- predict(model_radial_probs, newdata=scaledRISK_test_no_target, type = "prob")

# binarize output
testing_res_binary <- model.matrix(~ svm_actual - 1)


# get ROC curve and AUC for each class
roc_curves <- list()
for(i in 1:ncol(testing_res_binary)) {
  roc_curves[[i]] <- roc(testing_res_binary[,i], model_radial_probs_pred[,i])
}
