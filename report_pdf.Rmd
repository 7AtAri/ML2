...
rm(scores, sorted_scores, top_20_feature_names, feature_names)
```
\ 
...
# Use the function for a specific dataframe
RISK_train <- convert_columns_to_factors("RISK_train")
RISK_test <- convert_columns_to_factors("RISK_test")
...
```{r plot cross-val hpo results svm, echo=FALSE, cache=TRUE}
...
scaledRISK_test_no_target$Stratum <- NULL
scaledRISK_test_no_target$Prob <- NULL
svm_best_model_pred <- predict(svm_best_model, newdata=scaledRISK_test_no_target)
...
```{r rename target levels svm, include=FALSE, cache=TRUE}
# rename target levels 
levels(scaledRISK_train$suicidal_class) <- c("l1", "l2", "l3")
scaledRISK_train$suicidal_class <- factor(scaledRISK_train$suicidal_class, ordered = TRUE)
```
...
/ 
todo: try to make svm plot (link above from Ari)
