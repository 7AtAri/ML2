...
```{r plot cross-val hpo results, echo=FALSE, cache=TRUE, fig.width=11, fig.height=8}
...
model_linear <- tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmLinear", cost = C)
model_radial <- tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmRadial", cost = C, sigma = c(0.001, 0.01, 0.1))
model_poly <-  tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmPoly", cost = C,degree = c(2, 3, 4, 5), scale = c(0.1, 0.5, 1))
...
svm_best_model_pred <- predict(svm4$best.model, scaledRISK_test_no_target)
