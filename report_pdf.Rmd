...
\
What if we had just excluded every NA in the dataset? We will try and see if this is a viable option since this would not just be quick and easy, but we would also just have "real" answers. The exclusion of NAs leads to a severe reduction in the number of observations. The original data consisted of 17232 observations, if we reduce the target-related NAs only, we have 11753 observations left. If we omit all NAs, the reduced dataframe has 4334 observations.
\
...
```{r MCAR test for target columns, cache=TRUE, echo=FALSE, include=FALSE}
...
plot_correlation(RISK_red, geom_text_args = list(size = 0))
...
RISK_reduced$Stratum <- NULL 
RISK_reduced$Prob <- NULL
RISK_reduced$q7orig <- NULL
RISK_reduced$q6orig <- NULL
RISK_reduced$ID_unit <- NULL
#note: q6orig and q7orig do not have descriptions in the data manual and were discarded

# compute information gain
scores <- information.gain(suicidal_class ~ ., RISK_reduced)
...
feature_names <- setdiff(names(RISK_reduced), "suicidal_class")
...
rm(scores, sorted_scores, feature_names)
```
\ 
...
setdiff(corr_vars, names(named_top_20_features))
```

\

If we compare the features that came up in the correlation analysis and in the information gain calculations, we find only five meaningful variables that come up, "q40", "psu", "weight","q2" and "q76" .
...
final_vars<-c("q40", "q2", "psu", "weight", "q76", "suicidal_class", intersect(names(named_top_20_features), corr_vars))
...
# all variables highly correlated with target variable
RISK_red_new<-RISK_reduced[,unlist(final_vars)] 
...
```{r naming variables, cache=TRUE, echo=FALSE, warning=FALSE, results='asis', message=FALSE}
source("names.R") 
rm(significant_vars, corr_vars, named_top_20_features)
write_csv(RISK_red_new, "RISK_red_new.csv")
cat(paste0("- ", names(RISK_red_new), "\n", collapse = ""))

# knitr::kable(data.frame(variables=names(RISK_red)), align = "l")
...
Since we want to do a cross validation we will split into 80% Training and 20% Testing Data [[4]](#4).

```{r splitting the data, cache=TRUE, include=FALSE}
# import as dataframe
# RISK_red <- read_csv("RISK_red.csv", show_col_types = FALSE)
set.seed(0)  # set the seed
RISK_red_new <- read_csv("RISK_red_new.csv", show_col_types = FALSE)

print(sapply(RISK_red_new, class))
...
RISK_red_new <- convert_columns_to_factors("RISK_red_new")
...
model_linear <- tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmLinear", cost = C, scale=FALSE, cross=5, probability=TRUE)
model_radial <- tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmRadial",cross=5, cost = C,
                         gamma = gamma.range, scale=FALSE, probability=TRUE)
model_poly <-  tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmPoly", cross=5,cost = C,degree = c(1, 2, 3, 4), gamma = gamma.range, scale=FALSE, probability=TRUE)
...
```{r Confusion Matrix of Naive Bayes, echo=FALSE, cache=TRUE, warning=FALSE, results='asis'}
set.seed(0)
library(caret)
library(klaR)
library(e1071)
...
```
\
Given the Confusion Matrix, we can see an Accuracy of around 60.95% for the test data. The 95% confidence interval suggests that the true Accuracy of the model is between 55.99% and 65.74%. In this context, a p-value of less than 2.2e−16 for the hypothesis that "Accuracy is greater than No Information Rate" suggests that there is extremely strong statistical evidence that the accuracy of the model is better than what would be achieved by always random guessing (33%). This implies that the model has predictive power beyond mere chance and is effectively learning from the features in the dataset.
In the same time, we see that the system performs  better on extreme classes - no suicide risk and high suicide risk. This may suggest that the data we have is better suited for a binary classification problem, rather than multiclass.
\
##### 5.1.1.2 SVM

\

```{r Confusion Matrix of SVM,echo=FALSE, cache=TRUE, include=TRUE, results='asis'}
set.seed(0) 
...
```
\
Given the Confusion Matrix, we can see an Accuracy of around 64.68% for the test data. The 95% confidence interval suggests that the true Accuracy of the model is likely between 59.79% and 69.35%. In this context, a p-value of less than 2.2e−16 for the hypothesis that "Accuracy is greater than No Information Rate" suggests that there is extremely strong statistical evidence that the accuracy of the model is better than what would be achieved by always random guessing (33%). This implies that the model has predictive power beyond mere chance and is effectively learning from the features in the dataset.
\

```{r plot svm, echo=FALSE, cache=TRUE, results='asis'}
...
\
\
Additionally, to the accuracy score, we also evaluate our model for Precision, Recall, and F1-score. These metrics are especially helpful for classification in medical-related fields, where it is important to look at Precision and Recall specifically.
We therefore use multi-class metrics [[5]](#5).
\

##### 5.1.2.1 Naive Bayes
\

```{r nb multiclass metrics, echo=FALSE, cache=TRUE, results='asis'}
library(mltest)
...
```
\
As shown before, the classifier performs better on the extreme classes, especially "no-risk" class. The F1 scores for all classes reflect a balance between recall and precision.
Suicide prevention is a topic that tends to favor, for understandable reasons, more sensitive toward suicide risk class classifiers over more specific ones. This implies that the model must either be simplified, not differentiating between low and high risk, or be further enhanced using better features or methods.
Naive Bayes in general tends to serve as a baseline model, as it, by design, cannot capture the relationships between features. As such, other options, such as SVM should be better than it.
...
```{r svm multiclass metrics, echo=FALSE, cache=TRUE, include=TRUE, results='asis'}
library(mltest)
...
```{r extract performance nb, echo=FALSE, cache=TRUE, include=FALSE}
# read the models:
model_nb <- readRDS("model_nb.rda")
...
To further visually assess our classifier we plot the ROC curves for our three classes.
To plot the ROC curves, we need to get the probabilities for our model predictions.
Since we set a seed, we can use our best model and its hyperparameters to get the probabilities. 
\
\newpage
...
```{r ROC curves nb, echo=FALSE, cache=TRUE, include=FALSE, warning=FALSE, message=FALSE, results='hide'}
library(pROC)
library(nnet)  
...
for(i in 1:ncol(testing_res_binary)) {
  roc_curves_nb[[i]] <- roc(testing_res_binary[,i], nb_model_pred_prob[,i])
}
roc_curves_nb
```


```{r plotting and saving the ROC curves nb, echo=FALSE, cache=TRUE,results='hide', message=FALSE}
# ** this code snipped is created with the help of AI** ##
# Set up the PNG output
png("roc_curves_nb.png", width = 800, height = 800)

# Plot the first ROC curve
plot(roc_curves_nb[[1]], main = "ROC Curves", col = "red")

# Add the next ROC curves with lines()
lines(roc_curves_nb[[2]], col = "blue")
lines(roc_curves_nb[[3]], col = "green")
...
```{r plot ROC nb, echo=FALSE, message=FALSE, results='hide', fig.width=15, fig.height=10}

library(magick)

# Read the image
img <- image_read("roc_curves_nb.png")

# Resize the image
img_resized <- image_resize(img, "1200x800")
...
```{r ROC curves, echo=FALSE,results='hide', message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
library(pROC)
library(nnet)  

# get predictions from best model
svm_best_model_probs_pred <- predict(svm_best_model, newdata=scaledRISK_test_no_target, probability=TRUE)

# extract probabilites
probabilities_svm <- attr(svm_best_model_probs_pred, "probabilities")
...
roc_curves <- list()
for(i in 1:ncol(testing_res_binary)) {
  roc_curves[[i]] <- roc(testing_res_binary[,i], probabilities_svm[,i])
}
...
```{r plotting and saving the ROC curves, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, results='hide'}
# ** this code snipped is created with the help of AI** ##
# Set up the PNG output
...
## ** end of AI created code snipped ** 
```
```{r plot ROC svm, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.width=15, fig.height=10}

library(magick)

# Read the image
img_svm <- image_read("roc_curves.png")

# Plot the image
plot(img_svm)
# Plot the image at a specific size
#plot(1, type = "n", xlab = "", ylab = "", xlim = c(0, ncol(img_svm)/72), ylim = c(0, nrow(img_svm)/72), xaxt = "n", yaxt = "n", bty = "n", asp = 1)
#rasterImage(img_svm, 0, 0, ncol(img_svm)/72, nrow(img_svm)/72)
...
> <a id="id_4">[4]</a> <https://cran.r-project.org/web/packages/splitTools/vignettes/splitTools.html>

> <a id="id_5">[5]</a> <https://rdrr.io/cran/mltest/man/ml_test.html>
