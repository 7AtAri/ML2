...
question: it would introduce information leakage to reduce the features before splitting the data, correct?
...
inds <- partition(RISK_2000_5$suicidal_class, p = c(train = 0.6, valid = 0.2, test = 0.2))
RISK_train <- RISK_2000_5[inds$train, ]
RISK_valid <- RISK_2000_5[inds$valid, ]
RISK_test <- RISK_2000_5[inds$test, ]
...
RISK_num <- as.data.frame(lapply(RISK_2000_5, as.numeric)) # convert all variables to numeric
...
# all highly correlated columns
RISK_red_vars<-RISK_2000_5[, corr_vars$var] 

```



#### 3.2.2 Feature Importance Algorithm
