...
RISK_imp <- RISK_imp %>% 
  mutate(suicidal_class = ifelse(q29 > 1,   5,
                          ifelse(q28 > 1,   5,
                          ifelse(q27 == 1,  4,
                          ifelse(q26 == 1,  3,
                          ifelse(q25 == 1,  2,
...
RISK_imp[, c("q25", "q26", "q27", "q28", "q29")] <- NULL  # removing the columns related to the target variable
```
...
class_proportions <- table(original_df$target_var) / nrow(original_df)

# Calculate the desired number of observations for each class in the smaller dataset
desired_sample_size <- 2000  # Total desired sample size
sample_sizes <- round(class_proportions * desired_sample_size)

# Create an empty dataframe to store the smaller balanced dataset
smaller_df <- data.frame()

# Iterate through each class and perform stratified sampling
for (class in names(sample_sizes)) {
  # Subset observations belonging to the current class
  class_subset <- original_df[original_df$target_var == class, ]
  
  # Check if there are enough non-imputed observations for this class
  if (nrow(class_subset) >= sample_sizes[class]) {
    # If there are enough, randomly select 'sample_sizes[class]' observations
    sampled_obs <- class_subset[sample(1:nrow(class_subset), size = sample_sizes[class]), ]
  } else {
    # If there are not enough, include all non-imputed observations for this class
    sampled_obs <- class_subset
  }
  
  # Append the selected observations to the smaller dataset
  smaller_df <- rbind(smaller_df, sampled_obs)
}

# Ensure that the smaller dataset contains exactly 2000 observations (if needed)
if (nrow(smaller_df) > desired_sample_size) {
  smaller_df <- smaller_df[sample(1:nrow(smaller_df), size = desired_sample_size), ]
}

# Now 'smaller_df' contains your smaller balanced dataset with non-imputed data
