...
Ideally a classifier is able to detect the class k which maximizes the conditional probability $P(Y=k|x_{1},...,x_{p})$. A Bayes Classifier would know these probabilities for each of the classes exactly, but usually it is only possible to approximate those. The Naive Bayes Classifier is one method of approximation. It  approximates by "naively" assuming the conditional independence of predictor variables. This leads to simpler calculations. The joint probability of two events A and B $P(A \cap B)= P(A \mid B) * P(B) $ can be simplified to $P(A \cap B)= P(A) * P(B) $under the independence assumption, since conditional independence means $P(A \mid B) = P(A)$.
...
shuffled_train <- RISK_train[sample(1:nrow(RISK_train)), ] 
shuffled_test <- RISK_test[sample(1:nrow(RISK_test)), ] 

scaledRISK_train <- scaleDataset(shuffled_train)
scaledRISK_test <- scaleDataset(shuffled_test)
```
...
train_control <- trainControl(method = "cv", number = 5, search = "grid", savePredictions = "final", returnResamp = "all")
...
svm_best_model <- svm_models[[best_model_name]]
...
\
###### 5.1.1.2 Naive Bayes
```{r Confusion Matrix of Naive Bayes, echo=FALSE}
...
###### 5.1.1.2 SVM 
```{r Confusion Matrix of SVM, echo=FALSE}
svm_actual <- scaledRISK_test$suicidal_class
scaledRISK_test_no_target <- scaledRISK_test[,-5]
svm_best_model_pred <- predict(svm_best_model, newdata=scaledRISK_test_no_target)

table(svm_best_model_pred, actual, dnn=c("Prediction", "Actual"))   
confusionMatrix(table(svm_best_model_pred, actual))
```
...
###### 5.1.2.2 SVM 
```{r Confusion Matrix of SVM, echo=FALSE}
svm_actual <- as.numeric(as.character(actual))
...
\
##### 5.3.2 SVM
```{r Train and Test curve of SVM, echo=FALSE}
...
plot_performance <- function(performance, title) {
    ggplot(data = performance$resampling, aes(x = Iter, y = Accuracy)) +
        geom_line() +
        geom_point() +
...
# Plot for each model
plot_performance(performance_linear, "Linear Kernel Performance")
plot_performance(performance_radial, "Radial Kernel Performance")
plot_performance(performance_poly, "Polynomial Kernel Performance")
```
