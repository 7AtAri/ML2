...
model_radial <- tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmRadial",cross=5, cost = C, gamma = gamma.range, scale=FALSE)
...
# Function to extract the best accuracy from each model
extract_best_accuracy <- function(model) {
    max_accuracy <- max(model$best.performance)
    print(max_accuracy)
    return(max_accuracy)
}
...
```{r ROC curves nb, include=FALSE, cache=TRUE }
library(pROC)
