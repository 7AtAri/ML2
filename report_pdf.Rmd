...
The variable "record" seems to be an ID for the observations. This also has to be deleted since IDs do not hold predictive information that can be used for machine learning. 
...
But imputing the target variable related data may be quite challenging and out of the scope of this project. We therefore decide to nevertheless drop the target related NAs.
...
`q19`, `q20`, `q24`, `q30`, `q34`, `q35`, `q36`, `q39`, `q41`, `q43`, `q44`, `q45`, `q46`, `q47`, `q64`, `q85`, `q93`, `q98`.
...
\
For the Naive Bayes we examine one option for hyperparameter optimization:
...
```{r rename target levels, include=FALSE, cache=TRUE}
...
Given the list of all resulting models, coming from the HPO of the SVM, we extract the best model for further inspection and results using Accuracy as our focused metric.
Since our classes are balanced, Accuracy is the preferred testing metric. 
...
```
Given the Confusion Matrix we can see a Accuracy of around 60% meaning the predictions are around 60% correct. 
---> todo:
...
```{r multiclass metrics, echo=FALSE, cache=TRUE}
# library(mltest)
...
svm_best_model_probs <- predict(svm_best_model, newdata=scaledRISK_test, type = "prob")
# predictions <- predict(model_radial, scaledRISK_test, type = "prob")

# binarize output
#testing_res_binary <- class.ind(as.factor(scaledRISK_test$suicidal_class))
...
for(i in 1:ncol(testing_res_binary)) {
  roc_curves[[i]] <- roc(testing_res_binary[,i], svm_best_model_probs[,i])
}

# Plotting ROC curves
plot.roc(roc_curves[[1]], col = "blue")
sapply(2:length(roc_curves), function(i) lines.roc(roc_curves[[i]], col = i))

legend("bottomright", legend = levels(scaledRISK_test$suicidal_class), col = 1:3, lwd = 2)
```


No epochs ---> no test train curves
```{r plot test train curves, echo=FALSE, cache=TRUE}

# plot and save the performace results
plot_AUC <- function(performance, title) {
  
    if (!"Iter" %in% names(performance$resampling)) {
        cat("Available columns are:", names(performance$resampling), "\n")
        stop("Column 'Iter' not found in the data frame")
    }
