...
In SVM something called "the kernel trick" is used to tackle non-linearity in the data. It works by mapping the original non-linear data into a higher-dimensional space where it becomes linearly separable, thus allowing for effective classification without explicitly performing the computationally expensive transformation.
\
...
#model_linear <- readRDS(file = "models_linear.rda")
#model_radial <- readRDS(file = "models_radial.rda")
#model_poly <- readRDS(file = "models_poly.rda")
...
model_poly <-  tune.svm(suicidal_class ~., data = scaledRISK_train, method = "svmPoly", cross=5,cost = C,degree = c(1, 2, 3), gamma = gamma.range, scale=FALSE, probability=TRUE)
...
# read the models:
model_linear <- readRDS("models_linear.rda")
model_radial <- readRDS("models_radial.rda")
model_poly <- readRDS("models_poly.rda")
...
png("roc_curves_nb.png", width = 400, height = 400)
...
```{r plot ROC nb, echo=FALSE, message=FALSE, results='hide', fig.width=12, fig.height=7}
...
# Resize the image
img_resized <- image_resize(img, "900x500")
# Plot the image
plot(img_resized)
```
...
# ** this code snipped is created with the help of AI** ##
# Set up the PNG output
png("roc_curves.png", width = 400, height = 400)
...
## ** end of AI created code snipped ** 
```
```{r plot ROC svm, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.width=12, fig.height=7}
...
You can find our source code on GitHub [[6]](#6). 
...
> <a id="id_6">[6]</a> <https://github.com/7AtAri/ML2>
